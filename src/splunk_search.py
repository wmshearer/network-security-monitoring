...
and print/save JSON results. Same pattern as
splunk-detection-lab/src/splunk_search.py (that project is done and not
touched here), reused because SPL containing backslashes/quotes/wildcards is
fragile to shell-escape correctly from bash, and a misquoted search silently
returns an empty result set rather than erroring loudly.
